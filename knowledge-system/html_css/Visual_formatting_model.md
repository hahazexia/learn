...
内联级元素生成**内联盒子**，它们是参与`内联格式上下文`（inline formatting context）的框。
...
上例中 p 生成一个块盒子，其内部有几个行内盒子。“强调”的盒子是由内联元素 em 生成的`内联盒子`，但其他盒子（“Some” 和 “text”）是由块级元素 p 生成的内联框。后者称为`匿名内联盒`，因为它们没有关联的`内联级元素`。
